...
## Purpose
Owns: the dev-facing chain-builder GUI — chain visualization/reordering (`ChainContainer`, `EffectBlockComponent`), the parameter drawer (`ParameterPanel`), I/O routing tiles (`IOSelectorBlock`), TONE3000/preset/model dialogs, the overlay/card system (`OverlayHost`), look-and-feel (`PedaleiraLookAndFeel`), and `MainComponent` — the one persistent owner of every `EffectProcessor` in the live chain.
...
| Chain columns capped at 8 regardless of window width; IN always row 0, OUT always the last row with real content (dynamic, not fixed); rows past real content still render a dim decorative "+ ---- +" placeholder up to maxVisibleRows; row transitions draw as ONE continuous line (gutter stub + `ChainContainer`'s own full-width seam + gutter stub, same Y throughout) | Reads as a fixed pedalboard grid whose full capacity is visible from the moment the app opens, not something that grows unpredictably as you add effects, and a row's end visibly connects to the next row's start instead of two disconnected glyphs — both per explicit user correction 2026-09-10 | An unbounded column count that just fit the window width; two separate unconnected "continues" glyphs (a first attempt, corrected same day) |
